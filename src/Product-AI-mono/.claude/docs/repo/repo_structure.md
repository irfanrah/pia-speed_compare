...
│       └─ AI/                      #    모든 AI 로직이 여기에 있음
│
├─ assets/                          # 모델 가중치 & 테스트 영상 (Git 추적 안 됨)
...
│  ┌─────────────────────── 데이터 모델 ──────────────────┐
├─ DTO/                     │                               │
│  ├─ param_base.py         │ CategoryBase (파라미터 기본형)│
...
│  ├─ hyundai_esfalldown/   │ 현대 - 에스컬레이터 낙상      │
...
│  ┌─────────────────────── 유틸리티 ─────────────────────┐
├─ utils/                   │                               │
│  ├─ init.py               │ 인프라 초기화 (Logger/Redis/  │
│  │                        │   RabbitMQ/S3)                │
│  ├─ utils.py              │ 헬퍼 함수 (파라미터 변환,     │
│  │                        │   ROI 추출, threshold 체크)   │
...
```
[유형 1] OD 단독 ─────────────────────────────────────────────
  대표: ax_fall, samsung_fire, kumho_pinch
...
[유형 2] OD + ROI 기반 판정 ───────────────────────────────────
  ...
  프레임 → ROI 크롭 → OD 추론 → ROI 내부 존재 여부 판정 → 이벤트 → 알람

[유형 3] OD + Classification ──────────────────────────────────
  대표: khonkaen_helmet, khonkaen_weapon
...
| 9 | `khonkaen_weapon` | WeaponService | 콘깬 | 무기 소지 | OD+CLS |
| 10 | `kumho_pinch` | PinchService | 금호타이어 | 협착 | OD |
| 11 | `perception_encoder` | PEService | 공통 | PE 범용 | PE(Retrieval) |
| 12 | `pe_violence` | PVService | 공통 | 폭력 | PE(Retrieval) |
| 13 | `samsung_fire` | FireService | 삼성 | 화재 | OD |
