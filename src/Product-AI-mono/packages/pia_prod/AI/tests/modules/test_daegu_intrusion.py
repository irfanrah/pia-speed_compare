...
from pia_prod.AI.modules.Daegu_intrusion.service import IntrusionService
        ...
        add_stream_instance = AddStreamModel(
            ...
            cvEvent=[
                IntrusionModel(
                    ...
                    roi=ROIModel(
                        roiId=1,
                        polygonCoordinates=[1166, 631, 1512, 560, 1810, 867, 1271, 1006, 1071, 833],
                    ),
                )
            ],
...
def test_daegu_intrusion(model_download, local_video_path, get_AddStreamModel):
    ...
    if cap.isOpened():
        service = IntrusionService(q)
        service.logging_flag = True
        service.save_video = True
        service.video_manager = VideoSaveManager(
            output_dir=os.path.join(IMAGE_SAVE_PATH, stream_id, "intrusion"),
...
def test_daegu_intrusion_batches(
...
def test_import_module():
    ...
    from pia_prod.AI import IntrusionService

    assert IntrusionService is not None, "IntrusionService module import failed"
