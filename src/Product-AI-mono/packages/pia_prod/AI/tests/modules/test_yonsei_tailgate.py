...
from pia_prod.AI.modules.yonsei_tailgate.service import TailgateService
...
def get_AddStreamModel() -> tuple:
    def _create_add_stream_model(camera_id: int) -> tuple:
        ...
        assert TAILGATE_CV_KEY in user_param[USER_PARAM_KEY][CV_EVENT_KEY]
        cv_event = user_param[USER_PARAM_KEY][CV_EVENT_KEY]
        assert OD_TEST_THRESHOLD == cv_event[TAILGATE_CV_KEY][OD_THRESHOLD_KEY]
        assert IOU_TEST_THRESHOLD == cv_event[TAILGATE_CV_KEY][IOU_THRESHOLD_KEY]
...
def test_tailgate_4batch(get_od_model, get_video_1, get_video_2, get_AddStreamModel):
    cap_1 = cv2.VideoCapture(get_video_1)
...
def test_import_module():
    ...
    from pia_prod.AI import TailgateService

    assert TailgateService is not None, "TailgateService module import failed"
