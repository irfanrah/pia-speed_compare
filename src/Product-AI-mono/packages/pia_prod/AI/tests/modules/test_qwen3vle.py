...
from pia_prod.AI.modules.qwen3vle.service import Qwen3VLEService
...
def check_qwen3vle_embedding_server():
    """Probe /health; skip module if the embedding server is not reachable.

    Qwen3VLEService is a remote-engine client — it POSTs JPEG-encoded frame
    batches to the vLLM container at QWEN3VLE_EMBEDDING_SERVER_URL. Without
    that container running, every detect call would surface as ConnectError
    and the tests would fail for the wrong reason.
    """
    base_url = os.getenv("QWEN3VLE_EMBEDDING_SERVER_URL", QWEN3VLE_EMBEDDING_SERVER_URL)
    try:
        ...
        pytest.skip(
            "Qwen3VLE embedding server is not running. Start it with:\n"
            "  cd packages/pia_prod/AI/modules/qwen3vle/embedding_server\n"
            "  docker compose up -d --build"
        )
    if not data.get("model_loaded"):
        pytest.skip(
            "Qwen3VLE embedding server is up but vLLM hasn't finished loading. "
            "Wait for the 'Qwen3VLE embedding server ready' log line."
        )
    return base_url
...
def test_deterministic_qwen3vle_single_camera(
    setup_model_and_video, check_qwen3vle_embedding_server,
):
    """
    ...
    Requires the qwen3vle/embedding_server container to be running.
    """
...
def test_qwen3vle_single_camera(setup_model_and_video, check_qwen3vle_embedding_server):
    """
...
def test_qwen3vle_multiple_cameras(setup_model_and_video, check_qwen3vle_embedding_server):
    """
...
def test_qwen3vle_torch_one_camera(setup_model_and_video, check_qwen3vle_embedding_server):
    """
...
def test_qwen3vle_torch_two_cameras(setup_model_and_video, check_qwen3vle_embedding_server):
    """
