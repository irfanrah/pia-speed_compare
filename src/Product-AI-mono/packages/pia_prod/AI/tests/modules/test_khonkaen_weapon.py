...
from pia_prod.AI.modules.khonkaen_weapon.config import (
    ...
    WEAPON_DETECTION_MODEL_ONNX_PATH,
...
def get_pgie_model(hf_downloader):
...
@pytest.fixture(scope="module")
def get_sgie_model(hf_downloader):
    # model download from huggingFace
    model_path = WEAPON_DETECTION_MODEL_ONNX_PATH
    file_name = os.path.basename(model_path)
    repo_id, _ = os.path.splitext(file_name)
    hf_downloader.download(repo_id=repo_id, file_name=file_name, snapshot=False)
    assert WEAPON_DETECTION_MODEL_ONNX_PATH is not None, "Please, check model file path"
    # export model to TRT
    onnx2trt(
        onnx_path=model_path,
        device=0,
    ...
    assert os.path.exists(
        WEAPON_DETECTION_MODEL_TRT_PATH
    ), f"TRT model file {WEAPON_DETECTION_MODEL_TRT_PATH} not found"
        ...
        add_stream_instance = AddStreamModel(
            ...
            cvEvent=[
                WeaponModel(
                    ...
                    roi=ROIModel(
                        roiId=1,
                        polygonCoordinates=[0, 0, 0, 1080, 1920, 1080, 1920, 500],
                    ),
                )
            ],
...
def test_weapon_batches(
    get_pgie_model, get_sgie_model, get_video_1, get_video_2, get_AddStreamModel, batch_size=4
):
    cap_1 = cv2.VideoCapture(get_video_1)
...
def test_import_module():
    ...
    assert WeaponService is not None, "WeaponService import failed"
