...
from pia_prod.AI.modules.ax_fall.service import FallService
from pia_prod.AI.modules.ax_fall.config import FALL_DETECTION_MODEL_ONNX_PATH, OD_INPUT_SIZE
...
def model_download(hf_downloader):
    assert FALL_DETECTION_MODEL_ONNX_PATH is not None, "Please, check model file path"
    model_path = FALL_DETECTION_MODEL_ONNX_PATH
    file_name = os.path.basename(model_path)
    repo_id, _ = os.path.splitext(file_name)
    hf_downloader.download(repo_id=repo_id, file_name=file_name, snapshot=False)
    onnx2trt(
        onnx_path=FALL_DETECTION_MODEL_ONNX_PATH,
        device=0,
        fp16=True,
        overwrite=False,
...
def test_ax_fall(model_download, local_video_path):
...
def test_ax_fall_8batch(model_download, local_video_path):
    """
    단일 영상의 동일 프레임을 8개의 서로 다른 스트림(=배치 8)으로 전달하여
    FallService의 멀티 배치 처리를 검증한다.
    """
    cap = cv2.VideoCapture(local_video_path)
...
def test_import_module():
    """모듈 임포트 테스트"""
    from pia_prod.AI import FallService

    assert FallService is not None, "FallService 임포트 실패"
