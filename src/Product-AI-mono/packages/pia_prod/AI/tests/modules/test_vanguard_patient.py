...
from pia_prod.AI.modules.vanguard_patient.service import PatientService
from pia_prod.AI.modules.vanguard_patient.config import (
    MODEL_PATIENT_DETECTION_ONNX_PATH,
    MODEL_CLOTHCOLOR_CLS_ONNX_PATH,
    OD_INPUT_SIZE,
...
def model_download(hf_downloader):
    assert MODEL_PATIENT_DETECTION_ONNX_PATH is not None, "Please, check model file path"
    model_path = MODEL_PATIENT_DETECTION_ONNX_PATH
...
@pytest.fixture(scope="function")
def cls_model_download(hf_downloader):
    assert MODEL_CLOTHCOLOR_CLS_ONNX_PATH is not None, "Please, check model file path"
    model_path = MODEL_CLOTHCOLOR_CLS_ONNX_PATH
    file_name = os.path.basename(model_path)
    repo_id, _ = os.path.splitext(file_name)
    hf_downloader.download(repo_id=repo_id, file_name=file_name, snapshot=False)
    onnx2trt(
...
def local_video_path(nas_downloader, video_save_dir):
    video_name = "red_zone_patient.mp4"
...
def test_vanguard_patient(model_download, cls_model_download, local_video_path, get_AddStreamModel):
...
def test_vanguard_patient_batches(
    model_download, cls_model_download, local_video_path, get_AddStreamModel, batch_size=4
):
...
def test_import_module():
    ...
    from pia_prod.AI import PatientService

    assert PatientService is not None, "PatientService module import failed"
