...
from pia_prod.AI.modules.qwen3vle_trt.service import Qwen3VLETrtService
...
def setup_model_and_video(hf_downloader, nas_downloader, video_save_dir):
    hf_downloader.download(
        repo_id=QWEN3VLE_TRT_ID,
        snapshot=True,
    )

    # Builds the TRT engines only if they don't already exist in QWEN3VLE_TRT_ONNX_DIR_PATH;
    # otherwise this is a no-op and the existing Vision.engine / Transformer.engine are reused.
    export_to_trt(
        onnx_dir=QWEN3VLE_TRT_ONNX_DIR_PATH
    )
    ...
    debug_outvideo_path_1 = os.path.join(video_save_dir, "debug_qwen3vle_trt_out_1.mp4")
    debug_outvideo_path_2 = os.path.join(video_save_dir, "debug_qwen3vle_trt_out_2.mp4")
...
def _make_user_param(camera_id: str, camera_url: str, organization: str):
    """
    Qwen3 VL Embedding supports multiple categories: fire, smoke.
...
def test_import_module():
    ...
    assert Qwen3VLETrtService is not None, "Qwen3VLETrtService module import failed"


def test_deterministic_qwen3_vle_trt_single_camera(setup_model_and_video):
    """
    ...
    - Validates multi-category predictions (fire, smoke)
    """
    ...
    while True:
        ...
        if frame_count % interval == 0:
            ...
            if detect_out is not None:
                ...
                # so one stream can fire several keys in the same frame (e.g. fire + smoke).
    ...
    assert (
        len(decisions) > 0
    ), "Qwen3VLETrtService never produced a decision. (No Anomaly found)"

    # Service shutdown and GPU cleanup
    stop_thread(q)


def test_qwen3_vle_trt_single_camera(setup_model_and_video):
    """
    For a single stream, put frames at regular intervals into the queue and verify that Qwen3VLETrtService performs inference/event logic in a thread.
    Verify that multi-category (fire, smoke) predictions are processed correctly.
    """
    paths = setup_model_and_video
    video_path = paths["video1"]
    cap = cv2.VideoCapture(video_path)
...
def test_qwen3_vle_trt_multiple_cameras(setup_model_and_video):
    """
...
def test_qwen3_vle_trt_torch_one_camera(setup_model_and_video):
    """
    Single camera stream test:
    Convert Input Data to torch.Tensor (H, W, C, uint8) format rather than numpy array
    and verify that Qwen3VLETrtService handles Tensor input correctly.
...
def test_qwen3_vle_trt_torch_two_cameras(setup_model_and_video):
    """
    Multi-camera (2) stream test:
    Convert frames from two streams to torch.Tensor (H, W, C), put them in a batch list,
    and pass to Qwen3VLETrtService to verify multi-batch tensor processing.
...
# =====================================================================
# Server-level tests (Docker-hosted FastAPI / TRT inference endpoint)
#
# These hit the running container at QWEN3VLE_TRT_API_HOST/PORT and skip
# cleanly when the server is not reachable, so the suite stays runnable
# in environments without GPU + TRT engines.
# =====================================================================
@pytest.fixture(scope="module")
def check_qwen3vle_trt_server():
    """Probe /health; skip module if the server is not reachable."""
    host = os.getenv("QWEN3VLE_TRT_API_HOST", "localhost")
    port = os.getenv("QWEN3VLE_TRT_API_PORT", "9002")
    ...
    try:
        ...
        pytest.skip(
            "Qwen3VLE TRT server is not running. Start it with:\n"
            "  cd packages/pia_prod/AI/modules/qwen3vle_trt/server\n"
            "  docker compose up -d --build"
        )
    if not data.get("model_loaded"):
        pytest.skip(
            "Qwen3VLE TRT server is up but the TRT engine is not loaded yet. "
            "Wait for `docker compose logs` to show 'Qwen3VLETrtModel ready'."
        )
    return base_url


def _frame_to_b64(frame_bgr: np.ndarray) -> str:
    """BGR uint8 ndarray (cv2 native) → base64 JPEG. The server's PIL decoder
    reads the JPEG back as RGB, restoring the natural color order."""
    ok, buf = cv2.imencode(".jpg", frame_bgr)
    assert ok, "cv2.imencode failed"
    return base64.b64encode(buf.tobytes()).decode("ascii")


def test_qwen3_vle_trt_server_health(check_qwen3vle_trt_server):
    """Server is up and the model is loaded."""
    base_url = check_qwen3vle_trt_server
    resp = httpx.get(f"{base_url}/health", timeout=5)
...
def test_qwen3_vle_trt_server_embedding_single_frame(
    check_qwen3vle_trt_server, setup_model_and_video,
):
    """POST a single real frame to /v1/embeddings; validate response shape and
    that the returned embedding is a finite, non-degenerate float vector."""
    base_url = check_qwen3vle_trt_server
    paths = setup_model_and_video

    cap = cv2.VideoCapture(paths["video1"])
    try:
        assert cap.isOpened(), f"Can't open video: {paths['video1']}"
        ok, frame = cap.read()
        assert ok and frame is not None, "Failed to read first frame"
    finally:
        cap.release()

    payload = {"frames": [_frame_to_b64(frame)]}
    resp = httpx.post(f"{base_url}/v1/embeddings", json=payload, timeout=60)
    assert resp.status_code == 200, f"unexpected status: {resp.status_code} body={resp.text}"

    body = resp.json()
    assert "embedding" in body and "dim" in body, f"unexpected response shape: {body.keys()}"

    emb = np.asarray(body["embedding"], dtype=np.float32)
    assert emb.ndim == 1, f"embedding must be 1-D, got shape {emb.shape}"
    assert emb.size == body["dim"] > 0, f"dim mismatch: dim={body['dim']} len={emb.size}"
    assert np.all(np.isfinite(emb)), "embedding contains NaN/Inf"
    norm = float(np.linalg.norm(emb))
    assert norm > 1e-3, f"embedding looks degenerate (||e||={norm:.3e})"


def test_qwen3_vle_trt_server_embedding_multi_frame(
    check_qwen3vle_trt_server, setup_model_and_video,
):
    """POST a small T-frame video clip to /v1/embeddings; the server should
    accept the multi-frame request and return a single pooled embedding."""
    base_url = check_qwen3vle_trt_server
    paths = setup_model_and_video

    T = 4  # frames per request — small enough to keep the test snappy
    frames_b64 = []
    cap = cv2.VideoCapture(paths["video1"])
    try:
        assert cap.isOpened()
        ...
        cap.release()
    assert len(frames_b64) == T, f"expected {T} frames, got {len(frames_b64)}"
    ...
    body = resp.json()
    emb = np.asarray(body["embedding"], dtype=np.float32)
    assert emb.size == body["dim"] > 0
    assert np.all(np.isfinite(emb))


def test_qwen3_vle_trt_server_rejects_empty_frames(check_qwen3vle_trt_server):
    """The server validates `frames` is non-empty (FastAPI 400 path)."""
    base_url = check_qwen3vle_trt_server
    resp = httpx.post(f"{base_url}/v1/embeddings", json={"frames": []}, timeout=10)
    assert resp.status_code == 400, f"expected 400 for empty frames, got {resp.status_code}"


@pytest.mark.parametrize("concurrency", [4, 8])
def test_qwen3_vle_trt_server_concurrent_requests_dont_crash(
    check_qwen3vle_trt_server, setup_model_and_video, concurrency,
):
    """N parallel POSTs survive the engine lock — no crash, no 500s."""
    base_url = check_qwen3vle_trt_server
    paths = setup_model_and_video
    ...
    def _one_request(client: httpx.Client):
        t0 = time.perf_counter()
        resp = client.post(f"{base_url}/v1/embeddings", json=payload, timeout=60)
        return resp.status_code, resp.json() if resp.status_code == 200 else resp.text, time.perf_counter() - t0

    # httpx.Client with enough connections to allow real parallelism.
    # If the lock weren't there, this is the workload that crashed the server.
    with httpx.Client(limits=httpx.Limits(max_connections=concurrency)) as client:
        with ThreadPoolExecutor(max_workers=concurrency) as ex:
            results = list(ex.map(lambda _: _one_request(client), range(concurrency)))

    statuses = [r[0] for r in results]
    bodies = [r[1] for r in results]
    durations = [r[2] for r in results]

    # Every request succeeded (no 500s from the engine, no dropped connections).
    assert all(s == 200 for s in statuses), (
        f"concurrent requests should all return 200, got statuses={statuses}, "
        f"first failure body={next((b for s, b in zip(statuses, bodies) if s != 200), None)}"
    )

    # All embeddings are well-formed.
    dims = {b["dim"] for b in bodies}
    assert len(dims) == 1, f"all responses should agree on embedding dim, got {dims}"
    for body in bodies:
        emb = np.asarray(body["embedding"], dtype=np.float32)
        assert emb.size == body["dim"] > 0
        assert np.all(np.isfinite(emb))

    # The lock serializes engine work, so total wall time should be ~roughly
    # concurrency × single_request_latency. We don't assert tightly on this
    # (network + warmup variance is high), just that it didn't somehow finish
    # impossibly fast (which would suggest the lock is missing and one of the
    # "successful" responses was actually corrupt).
    assert max(durations) > 0, "duration tracking failed"

    # Server is still up (didn't crash and auto-restart mid-test).
    health = httpx.get(f"{base_url}/health", timeout=5)
    assert health.status_code == 200 and health.json().get("model_loaded"), (
        "server health degraded after concurrent load — engine likely crashed."
    )
