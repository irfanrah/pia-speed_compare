from pia_prod.AI.modules.pe_binary.service import PEBinaryService
from pia_prod.AI.modules.pe_binary.config import (
    PE_BINARY_TXT_FEATURE_PATH,
    PE_BINARY_TRT_PATH,
    PE_BINARY_ONNX_PATH,
    PE_BINARY_PYTORCH_PATH,
    INPUT_SIZE,
)
...
from pia_prod.AI.modules.pe_binary.trt_export import export_trt_engine
import pytest
import cv2
import os
...
def setup_model_and_video(hf_downloader, nas_downloader, video_save_dir):
    txt_json_name = os.path.basename(PE_BINARY_TXT_FEATURE_PATH)
    onnx_file_save_path = PE_BINARY_ONNX_PATH
    repo_name = os.path.splitext(os.path.basename(PE_BINARY_PYTORCH_PATH))[0]
    ...
    export_trt_engine(
        ...
        save_file_path=PE_BINARY_TRT_PATH,
        min_batch_size=1,
        opt_batch_size=4,
        max_batch_size=8,
    ...
    video_name_1 = "pe_binary_KTT_coupang_falldown_fire_smoke_video.mp4"
    video_name_2 = "pe_binary_esfalldown_video.mp4"
    video_name_3 = "pe_binary_elvfalldown_video_1.mp4"
    # video_name_4 = "pe_binary_elvfalldown_video_2.mp4"
...
def test_pe_binary(setup_model_and_video):
...
def test_pe_binary_without_falldown(setup_model_and_video):
    video_path_1, video_path_2, video_path_3 = setup_model_and_video
...
def test_pe_binary_multiple_cameras(setup_model_and_video):
    video_path_1, video_path_2, video_path_3 = setup_model_and_video
...
def test_import_module():
    ...
    from pia_prod.AI import PEBinaryService

    assert PEBinaryService is not None, "PEBinaryService import failed"


def test_pe_torch_one_camera(setup_model_and_video):
    # model download from huggingFace
    # TODO : 현재는 pia-ai-package에 있는 모델 경로로만 실행 가능
    # assert os.getenv("PERCEPTION_ENCODER_PYTORCH_PATH") != "", "Please, check model file path"
...
def test_pe_torch_two_cameras(setup_model_and_video):
    video_path_1, video_path_2, video_path_3 = setup_model_and_video
    cap_1 = cv2.VideoCapture(video_path_1)
    cap_2 = cv2.VideoCapture(video_path_2)
    q = Queue(1)
...
def test_pe_binary_elvfalldown(setup_model_and_video):
    video_path_1, video_path_2, video_path_3 = setup_model_and_video
    ...
    user_param["user_param"] = AddStreamModel2dict(
        AddStreamModel(
            ...
            retEvent=[
                RetrievalBase(
                    name="elvfalldown_ret",
                    incidentThresholdSecond=3,
                    incidentTimeoutSecond=3,
                    roi=ROIModel(roiId=1, polygonCoordinates=[]),
                    topCandidates=5,
                    abnormalText=["elvfalldown"],
                    normalText=["normal"],
                ),
            ],
            timestamp=str_UTC_ISO8601_ms_now_time(),
