...
from pia_prod.AI.modules.glenc_harness.service import HarnessWithClsService
...
from pia_prod.AI.modules.glenc_harness.config import (
    ...
    MODEL_HARNESS_CLS_TRT_PATH,
    MODEL_HARNESS_CLS_ONNX_PATH,
    DEFAULT_FPS,
...
from pia_prod.AI.modules.glenc_harness.debug_utils import VideoSaveManager
...
def get_cls_model(hf_downloader):
    # model download from huggingFace
    assert MODEL_HARNESS_CLS_ONNX_PATH is not None, "Please, check model file path"
    model_path = MODEL_HARNESS_CLS_ONNX_PATH
    file_name = os.path.basename(model_path)
    repo_id, _ = os.path.splitext(file_name)
    hf_downloader.download(repo_id=repo_id, file_name=file_name, snapshot=False)
    # export model to TRT
    ...
    assert os.path.exists(
        MODEL_HARNESS_CLS_TRT_PATH
    ), f"TRT model file {MODEL_HARNESS_CLS_TRT_PATH} not found"
...
def get_video_2(nas_downloader, video_save_dir):
    video_name = "glenc_harness_inlift_1.mp4"
    local_video_path = os.path.join(video_save_dir, video_name)
    nas_video_path = nas_downloader.get_nas_path(video_name)
    nas_downloader.download_file(nas_video_path, local_video_path)
...
def test_harness_1batch(
    ...
    count = 0
    if cap_1.isOpened():
        service = HarnessWithClsService(q)
        service.save_video = True
        service.video_manager = VideoSaveManager(
            output_dir=os.path.join(IMAGE_SAVE_PATH, stream_id, "harness"),
...
def test_import_module():
    ...
    assert HarnessService is not None, "HarnessService module import failed"


@pytest.mark.parametrize("category_name", ["harnessoff_cv", "하네스미착용_cv"])
def test_addstream_routes_to_harness_with_cls_model(category_name):
    """영문/한글 카테고리 키 모두 HarnessWithClsModel 로 라우팅되는지 회귀 검증."""
    add_stream_instance = AddStreamModel(
        cameraId=1,
        cameraUrl="11",
        organization="pia",
    ...
    assert isinstance(add_stream_instance.cvEvent[0], HarnessWithClsModel)
    assert add_stream_instance.cvEvent[0].name == category_name
