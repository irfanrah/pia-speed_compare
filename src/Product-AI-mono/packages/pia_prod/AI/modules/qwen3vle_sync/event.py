...
from pia_prod.AI.modules.qwen3vle_sync.config import (
    CATEGORY_EVENT_MAP,
    QWEN3VLE_SYNC_QUEUE_SIZE,
    QWEN3VLE_SYNC_ALARM_DURATION_THRESHOLD,
)
from pia_prod.AI.global_config import USER_PARAM_KEY, RET_EVENT_KEY


class Qwen3VLESyncEventManager(EventBase):
    """
    Multi-category Event Manager — identical to Qwen3VLEEventManager in
    `qwen3vle/event.py`, but reads its tuning constants from
    `qwen3vle_sync.config` so the two modules can be tuned independently
    (e.g. shorter alarm duration when running in-process for lower latency).
    ...
    - Alarms are keyed by f"{stream_id}__{category_id}" so multiple categories
      triggering on the same stream don't overwrite each other.
    """

    def __init__(self):
        super().__init__(alarm_duration=QWEN3VLE_SYNC_ALARM_DURATION_THRESHOLD)
        self.queue_size = QWEN3VLE_SYNC_QUEUE_SIZE
