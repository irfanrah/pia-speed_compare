...
from pia_prod.AI.modules.qwen3vle_sync.config import ALL_CATEGORIES, DEVICE


class Qwen3VLESyncRoIManager(ROIManagerBase):
    """
    Multi-category RoI Manager for the in-process (sync) Qwen3VLE service.
    Identical behavior to `qwen3vle.roi_manager.Qwen3VLERoIManager`; only
    the config import differs so each module can be tuned independently.
    """
    ...
    def process_batches_with_roi(
        ...
        for idx, (batch, user_param) in enumerate(zip(batches, user_params)):
            ...
            if found_key is None:  # no ROI → use full frame
