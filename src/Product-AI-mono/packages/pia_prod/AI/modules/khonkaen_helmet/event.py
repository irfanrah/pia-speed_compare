...
from pia_prod.AI.modules.khonkaen_helmet.config import HELMET_QUEUE_SIZE, HELMET_QUEUE_THRESHOLD


class HelmetEventManager(EventBase):
    def __init__(self):
        super().__init__()
        self.alert_message = "ALERT: Not Wearing Helmet detected!!"
        self.event_dict = defaultdict(lambda: deque(maxlen=HELMET_QUEUE_SIZE))
    ...
    def get_alarm(self, target, logging_flag=False) -> None:
        """
        이벤트 임계값을 초과한 카메라에 대해 알람을 트리거한다.

        Args:
            target (list): 분석할 대상 카메라 리스트.
        """
        for camera_id in target:
            now_status = bool(sum(self.event_dict[camera_id]) >= HELMET_QUEUE_THRESHOLD)  # False or True
