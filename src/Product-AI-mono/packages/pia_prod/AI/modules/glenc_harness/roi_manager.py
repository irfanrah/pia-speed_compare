...
from pia_prod.AI.modules.glenc_harness.config import (
    DEVICE,
    HARNESS_WITH_CLS_CV_CATEGORY,
)


class HarnessWithClsRoIManager(ROIManagerBase):
    """
    ...
    def __init__(self):
        ...
        self.category_list = HARNESS_WITH_CLS_CV_CATEGORY
    ...
    def process_batches_with_roi(
        ...
        for idx, (batch, user_param) in enumerate(zip(batches, user_params)):
            ...
            if found_key is None:
                raise ValueError(
                    f"None of {self.category_list} found in cvEvent: {list(cv_event.keys())}"
                )
