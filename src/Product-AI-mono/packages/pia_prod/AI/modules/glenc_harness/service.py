...
from pia_prod.AI.modules.glenc_harness.event import HarnessWithClsEventManager
from pia_prod.AI.modules.glenc_harness.roi_manager import HarnessWithClsRoIManager
...
from pia_prod.AI.modules.glenc_harness.debug_utils import save_snapshot_for_odcls
...
class HarnessWithClsService(ServiceBase):
    ...
    def _load_roi_manager(self):
        return HarnessWithClsRoIManager()
    ...
    def _load_model(self):
        ...
        self.classification_instance = PiaONNXTensorRTModel(
            model_path=MODEL_HARNESS_CLS_TRT_PATH, device=DEVICE
        )
    ...
    def _load_event_manager(self):
        return HarnessWithClsEventManager()
    ...
    def _detect(self, **datas):
        ...
        self.get_alarm_harness(cls_results, matched_info)  # Event Manager, Get Alarm

        alarms = {}
        for origin_batch_idx in categories_batches:  # Send Alarm
            state = self.alarm_event_manager.event_state[stream_ids[origin_batch_idx]]
            if state in [0, 2]:  # 알람이 없거나 계속 진행중일 땐 메세지를 보내지 않음
                continue
            is_start = state == 1  # 알람이 처음 생긴 경우 thumbnail을 보냄
    ...
    def get_pgie_results(
        ...
        #     for key in HARNESS_CV_CATEGORY:
        #         if key in cv_event:
    ...
    def get_alarm_harness(self, cls_results: List[bool], matched_info: Dict[str, int]) -> None:
        """
        ...
        Returns:
            None
        """
        for idx, (camera_id, _) in enumerate(matched_info.items()):
            self.alarm_event_manager.update(cls_results[idx], camera_id)

        self.alarm_event_manager.get_alarm(matched_info)
