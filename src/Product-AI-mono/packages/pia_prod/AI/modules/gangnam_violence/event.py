...
class VQAViolenceEventManager(EventBase):
    def __init__(self):
        super().__init__(
    ...
    def update(self, responses: List[str], categories_per_stream: List[str], stream_ids: List[str]):
        ...
        for response, categories, stream_id in zip(responses, categories_per_stream, stream_ids):
            for cat in categories:
                ...
                if predict in MODEL_OUTPUTS[0] and cat in VIOLENCE_CATEGORY:  # violence
        ...
        try:
            ...
            if start_brace != -1 and end_brace != -1 and start_brace < end_brace:
                ...
                try:
                    ...
                    pass
            cat_match = re.search(
                r'["\']category["\']\s*:\s*["\'](violence|normal)["\']', clean_str, re.IGNORECASE
            )
            if cat_match:
