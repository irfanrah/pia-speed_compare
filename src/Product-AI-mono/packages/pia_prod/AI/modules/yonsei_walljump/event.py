...
from pia_prod.AI.modules.yonsei_walljump.config import WALL_JUMP_QUEUE_SIZE, WALL_JUMP_THRESHOLD


class WalljumpEventManager(EventBase):
    def __init__(self):
        super().__init__()
        self.alert_message = "ALERT: Wall jump detected!!"
        self.event_dict = defaultdict(lambda: deque(maxlen=WALL_JUMP_QUEUE_SIZE))
    ...
    def get_alarm(self, target, logging_flag=False) -> None:
        ...
        for camera_id in target:
            now_status = bool(
                sum(self.event_dict[camera_id]) >= WALL_JUMP_THRESHOLD
            )  # False or True
