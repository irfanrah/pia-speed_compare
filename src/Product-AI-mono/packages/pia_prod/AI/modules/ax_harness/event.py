...
from pia_prod.AI.modules.ax_harness.config import (
    HARNESS_QUEUE_THRESHOLD,
    HARNESS_QUEUE_SIZE,
)


class HarnessEventManager(EventBase):
    def __init__(self):
        super().__init__()
        self.category_name = "harnessoff_cv"
        self.alarm_duration = HARNESS_QUEUE_THRESHOLD
        self.duration_queue = defaultdict(lambda: deque(maxlen=(HARNESS_QUEUE_SIZE)))
        self.event_status = defaultdict(int)

    def update(self, results, stream_ids, rois):
        # ROI 내에 들어갔는지 확인
        alarms = []
        for dets, stream_id in zip(results, stream_ids):
            cnt = 1 if len(dets) > 0 else 0
    ...
    def check_alarm_duration(self, target_cameras: list):
        ...
        for target_camera in target_cameras:
            ...
            is_over_queue = int(sum(value) >= HARNESS_QUEUE_THRESHOLD)
