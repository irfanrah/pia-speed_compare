...
from pia_prod.AI.modules.pe_violence.config import VIOLENCE_CATEGORY, DEVICE


class PEViolenceRoIManager(ROIManagerBase):
    """
    RoI 정보를 관리하고, 입력 이미지를 RoI 기반으로 잘라내는 매니저.

    - 카메라별 최신 RoI 좌표를 캐싱
    - 좌표가 변경되면 자동으로 갱신
    - RoI가 비어 있으면 전체 프레임을 대상으로 사용
    - 잘라낸 이미지를 후속 전처리 파이프라인에 전달
    """
