...
from pia_prod.AI.modules.Daegu_intrusion.roi_manager import IntrusionRoIManager
from pia_prod.AI.modules.Daegu_intrusion.event import IntrusionEventManager
...
from pia_prod.AI.modules.Daegu_intrusion.debug_utils import save_snapshot_for_od
...
class IntrusionService(ServiceBase):
    ...
    def _load_event_manager(self):
        self.intrusion_event_manager = IntrusionEventManager()
    ...
    def _detect(self, **datas):
        ...
        person_od_result, total_bbox_info, cropped_batches = self.get_pgie_results(
            batches=batches,
            stream_ids=stream_ids,
            user_params=user_params,
        )

        alarms = self.intrusion_event_manager(
    ...
    def get_pgie_results(
            ...
            for batch_idx, batch in enumerate(person_od_result):
                ...
                for pred in batch:
                    ...
                    origin_bbox = modified2origin_coordinate(
                        ...
                        original_shape=cropped_batches[batch_idx].shape[1:3],
                    )
        ...
        return person_od_result, total_bbox_info, cropped_batches
