...
from pia_prod.AI.modules.ax_fall.roi_manager import FallRoIManager
from pia_prod.AI.modules.ax_fall.event import FallEventManager
...
from pia_prod.AI.modules.ax_fall.config import (
    DEVICE,
    FALL_DETECTION_MODEL_TRT_PATH,
    LIMITED_NUM_OF_FALL_PER_CAMERA,
...
class FallService(ServiceBase):
    ...
    def _load_roi_manager(self):
        return FallRoIManager()

    def _load_model(self):
        self.fall_model = PiaONNXTensorRTModel(
            FALL_DETECTION_MODEL_TRT_PATH, device=load_model_backend(DEVICE)
        )

    def _load_event_manager(self):
        self.fall_event_manager = FallEventManager()
    ...
    def _detect(self, **datas):
        ...
        # model Inference
        raw_fall_od_result = self.fall_model(preprocessed_images)

        # NMS
        fall_od_result = non_max_suppression(
            raw_fall_od_result,
            conf_thres=OD_CONFIDENCE_THRESHOLD,
            agnostic=True,
            iou_thres=OD_NMS_THRESHOLD,
            classes=[0],
            max_det=LIMITED_NUM_OF_FALL_PER_CAMERA,
        )

        alarms = self.fall_event_manager(
            results=fall_od_result,
