...
class PeVqa2StageEventManager(EventBase):
    def __init__(self):
        super().__init__(
        ...
        for stream_id, cat_dict in self.duration_queue.items():
            for category_id, value in cat_dict.items():
                ...
                if now_status in [1, 3]:
                    # composite key — 동일 stream의 다중 카테고리 동시 전이 시 collision 방지.
                    # value의 category_id 자리는 ServiceBase.get_alarm_with_uuid가 다시 __category_id를
                    # 이중 prefix하지 않도록 빈 문자열로 비워둔다 (ft_pe 패턴과 동일).
