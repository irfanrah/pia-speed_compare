...
class CLIPVisionViPEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
        ...
        patch_embeds = self.patch_embedding(
            pixel_values.reshape(-1, C, H, W)
        )  # [B*T, C, H, W]
        patch_embeds = patch_embeds.flatten(2).transpose(
            1, 2
        )  # [B*T, C, H*W ] -> [B*T, H*W, C]
        C = patch_embeds.shape[-1]  # in default setting : C = 768
        patch_embeds = patch_embeds.reshape(B, T, -1, C)

        if self.if_use_temporal_embed:
            patch_embeds = patch_embeds + time_embed.unsqueeze(
                2
            )  # [B, T, H*W, C] - patch embeds에 시간에 대한 정보를 더해준다
...
class CLIPTextTransformer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.embeddings(
            input_ids=input_ids, position_ids=position_ids  # (B,50)
        )  # None

        bsz, seq_len = input_shape  # (B, 50, 512)
        ...
        encoder_outputs = self.encoder(
            inputs_embeds=hidden_states,  # (B, 50, 512)
            attention_mask=attention_mask,  # (B, 1, 50, 50)
            causal_attention_mask=causal_attention_mask,  # (B, 1, 50, 50)
...
class CLIPVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        if len(pixel_values.shape) == 6:  # tiled frames
            B, T, Fr, C, H, W = pixel_values.shape
            pixel_values = pixel_values.view(B * T, Fr, C, H, W)
        else:
...
class CLIPModel(CLIPPreTrainedModel):
    ...
    def forward(
        ...
        image_embeds = image_embeds.view(-1, image_embeds.size(-1))
