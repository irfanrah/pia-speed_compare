...
class CLIPVIP(T2VRetModelBase):
    ...
    def forward(
        ...
        if isinstance(text, str) or isinstance(text, list) or (txt_mask is None and text is not None):
        ...
        ret = self.model.forward(video=video, text_input_ids=text, text_input_mask=txt_mask)
    ...
    def export(self, onnx_config: T2VRetONNXConfig):
        ...
        if onnx_config.split_part == "visual":
            ...
            input_dummy = torch.randint(
                0,
                self.model.clipmodel.text_model.embeddings.token_embedding.num_embeddings - 1,
                input_size["input0"],
                dtype=torch.int32,
            ).to(
                self.config.device
...
class TextualModel(torch.nn.Module):
    def __init__(self, model: clip_vip) -> None:
        ...
        self._build_causal_attention_mask = model.clipmodel.text_model._build_causal_attention_mask

    def forward(self, x, mask):
        y_ex = _expand_mask(mask, x.dtype)
        if_fp16 = x.dtype == torch.float16
        bsz, seq_len = mask.size()  # (B, 50)
        causal_attention_mask = self._build_causal_attention_mask(bsz, seq_len, fp16=if_fp16).to(
            x.device
        )
        x = self.hidden_model(
            inputs_embeds=x,
