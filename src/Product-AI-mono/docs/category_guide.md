...
> **사전 준비**: [01_프로젝트_구조_가이드.md](./01_프로젝트_구조_가이드.md) 를 먼저 읽어주세요

---

## 1. 작업 흐름 전체 요약

신규 카테고리 추가는 아래 **7단계**를 순서대로 진행합니다.
테스트 코드를 먼저 작성하여 "무엇을 만들어야 하는지" 정의한 뒤, 구현을 채워넣는 흐름입니다.

```
┌──────────┐   ┌─────────┐   ┌──────────┐   ┌──────────┐   ┌──────────┐   ┌──────────┐   ┌──────────┐
│ STEP 1   │──→│ STEP 2  │──→│ STEP 3   │──→│ STEP 4   │──→│ STEP 5   │──→│ STEP 6   │──→│ STEP 7   │
│ 테스트     │   │ 모듈     │   │ 핵심 파일  │   │ 시스템     │   │ 모델 파일  │   │ 테스트     │   │ PR 제출   │
│ 코드 작성   │  │ 디렉토리  │   │ 구현      │   │ 등록      │    │ 배치      │   │ 추론      │   │          │
│          │   │ 생성     │   │ (6개 필수) │   │ (2개 수정)│    │          │   │          │   │          │
└──────────┘   └─────────┘   └──────────┘   └──────────┘   └──────────┘   └──────────┘   └──────────┘

  tests/에        신규 생성       신규 생성       기존 파일 수정     assets/에      테스트 실행     브랜치 →
  테스트 작성     1개 디렉토리    6개 파일        2개 파일           모델 배치      및 검증        커밋 → PR
```

**최종 결과**: 테스트 1개 파일 + 신규 6개 파일 생성 (필요시 추가 가능) + 기존 2개 파일 수정 = **총 최소 9개 파일**
...
│
├─ __init__.py                              [MOD] 3곳 수정 (STEP 4-1)
│
├─ DTO/
│  └─ stream_params.py                      [MOD] 4곳 수정 (STEP 4-2)
│
├─ modules/
│  └─ <새_모듈명>/                           [NEW] 디렉토리 생성
│     ├─ __init__.py                        [NEW] 빈 파일 (STEP 3-6)
│     ├─ config.py                          [NEW] 설정 상수 (STEP 3-1)
│     ├─ param.py                           [NEW] 파라미터 모델 (STEP 3-2)
│     ├─ event.py                           [NEW] 이벤트 매니저 (STEP 3-3)
│     ├─ service.py                         [NEW] 서비스 클래스 (STEP 3-4)
│     └─ roi_manager.py                     [NEW] ROI 매니저 (STEP 3-5)
│
└─ tests/modules/
   └─ test_<새_모듈명>.py                   [NEW] 테스트 코드 (STEP 1)
```
...
테스트 코드를 먼저 작성하여, 구현해야 할 서비스의 인터페이스와 동작을 정의합니다.
`conftest.py`에 정의된 공유 fixture(`hf_downloader`, `nas_downloader`, `video_save_dir`)를 활용합니다.
...
from pia_prod.AI.modules.<모듈명>.param import YourModel
...
#   conftest.py의 hf_downloader 공유 fixture를 사용합니다.
# ────────────────────────────────────────────────────────────
@pytest.fixture(scope="module")
def model_download(hf_downloader):
    assert MODEL_ONNX_PATH is not None, "Please, check model file path"
...
# ────────────────────────────────────────────────────────────
# Fixture: 테스트 영상 다운로드
#   conftest.py의 nas_downloader, video_save_dir 공유 fixture를 사용합니다.
# ────────────────────────────────────────────────────────────
@pytest.fixture(scope="module")
def local_video_path(nas_downloader, video_save_dir):
    video_name = "your_test_video.mp4"
...
# ────────────────────────────────────────────────────────────
# 테스트 3: 멀티 카메라 배치 추론 (batch_size=4)
# ────────────────────────────────────────────────────────────
def test_batches(model_download, local_video_path, batch_size=4):
    from pia_prod.AI import YourService

    cap = cv2.VideoCapture(local_video_path)
    q = Queue(1)

    # 카메라 4대분 user_param 및 stream_id 생성
    user_params = []
    stream_ids = []
    ...
    if cap.isOpened():
        ...
        while True:
            ...
            # 동일 프레임을 batch_size만큼 복제하여 4대 카메라 시뮬레이션
            batches = [frame] * batch_size
...
**conftest.py에서 제공하는 공유 fixture:**

```
┌──────────────────────────────────────────────────────────────────────┐
│ conftest.py (packages/pia_prod/AI/tests/conftest.py)                │
├──────────────────────┬───────────────────────────────────────────────┤
│ fixture              │ 설명                                         │
├──────────────────────┼───────────────────────────────────────────────┤
│ hf_downloader        │ HuggingFace 모델 다운로더 (scope=session)    │
│ nas_downloader       │ NAS 파일 다운로더 (scope=session)            │
│ video_save_dir       │ "assets/videos" 경로 반환 (scope=session)    │
└──────────────────────┴───────────────────────────────────────────────┘
```
...
## 5. STEP 3 - 핵심 파일 구현 (6개 필수)

### 5.1 `config.py` - 설정 상수 정의
...
```
                        ...
                        False(0)  True(1)
                        ┌────────┬────────┐
  현재상태 0 (no_event)   │ → 0    │ → 1 ★  │  ★ = 알람 발생 (1: 이벤트 시작, 3: 이벤트 종료)
                        ├────────┼────────┤
  현재상태 1 (True)       │ → 3 ★  │ → 2    │
                        ├────────┼────────┤
  현재상태 2 (continue)   │ → 3 ★  │ → 2    │
...
### 5.4 `service.py` - 서비스 클래스 구현 (핵심 파일)
...
class YourService(ServiceBase):
    ...
    def __init__(self, analysis_data_queue):
        ...
        # ※ 부모 생성자가 _init_values(), _load_model(), _load_roi_manager(),
        #   _load_event_manager()를 순서대로 호출합니다.
        #   이 중 _load_roi_manager()는 @final이지만 런타임에서 강제되지 않으므로
        #   대부분의 모듈에서 오버라이드합니다.
    ...
    # ──────────────────────────────────────────────────────
    # ROI 매니저 오버라이드
    #   기본 ServiceBase는 ROIManagerBase()를 반환합니다.
...
```
성공 시:  {
            ALARMS_KEY:              dict  ← {stream_id: [True/False, category_id]}
            BATCHES_KEY:             list  ← 원본 batches (썸네일 추출용)
            STREAM_IDS_KEY:          list  ← 원본 stream_ids (변경 없이 전달)
            USER_PARAMS_KEY:         list  ← 원본 user_params (변경 없이 전달)
            IS_NEEDED_CVT_COLOR_KEY: bool  ← BGR→RGB 변환 필요 여부
          }
...
참고 - 이벤트 타입별 대표 모듈:
  ...
  VQA: gangnam_falldown, gangnam_violence
```

---

### 6.1 `AI/__init__.py` 수정 (3곳) - 모든 이벤트 타입 공통
...
### 6.2 `DTO/stream_params.py` 수정 - cvEvent인 경우만 (4곳)
...
### 6.3 이벤트 타입별 param.py & 서비스 내부 차이
...
│ 참고 모듈:    gangnam_falldown, gangnam_violence                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
> 테스트 fixture의 `hf_downloader`가 HuggingFace에서, `nas_downloader`가 NAS에서 자동 다운로드합니다.

---

## 8. STEP 6 - 테스트 추론

STEP 1에서 작성한 테스트 코드를 실행하여 구현이 정상 동작하는지 검증합니다.

```bash
# 단일 모듈 테스트 실행
pytest packages/pia_prod/AI/tests/modules/test_<모듈명>.py -v

# import 테스트만 빠르게 실행
pytest packages/pia_prod/AI/tests/modules/test_<모듈명>.py::test_import_module -v
```
...
```
 커밋순서(시간순)                                            대응 STEP
 ──────────────────────────────────────────────────────────────────────
 1  feat: addstream에 배회추가                               STEP 4 (등록)
 2  test: 배회 테스트코드 추가                                STEP 1 (테스트 코드)
 3  chore: 배회 관련 컨피그 정의                              STEP 3 (config.py)
 4  feat: 배회 이벤트매니저 정의                               STEP 3 (event.py)
 5  chore: 배회 모델의 param 정의                             STEP 3 (param.py)
 6  chore: od 모델 nms 함수 임시 추가                         선택 파일
 7  feat: 배회 roi manager 추가                              STEP 3 (roi_manager.py)
 8  feat: 배회 Service manager 개발                          STEP 3 (service.py)
...
```
 커밋순서(시간순)                                            대응 STEP
 ──────────────────────────────────────────────────────────────────────
 1  chore: 기본 파일 추가                                    STEP 2 (스캐폴딩)
 2  feat: PIA octracker 정의                                 선택 파일 (tracker.py)
 3  chore: 환자배회 초기 config.py 정의                       STEP 3 (config.py)
 4  feat: 환자배회 service 초안 작성                           STEP 3 (service.py)
 5  feat: 환자배회 roi_manager 초안 작성                      STEP 3 (roi_manager.py)
 6  test: 테스트 코드 초안 작성                                STEP 1 (테스트 코드)
 7  chore: stream_params에 patient 추가                      STEP 4 (등록)
 8  feat: 환자배회 이벤트 초안 작성                            STEP 3 (event.py)
 9  feat: 환자배회 param 초안 작성                             STEP 3 (param.py)
10  chore: _SERVICE_MODULE_OVERRIDE 에 patient 추가           STEP 4 (등록)
11  (이후 20+ 커밋으로 개선/수정)                               반복 개선
```

**공통 패턴:**

```
테스트 코드 작성 + 등록 (초반) → config/param 정의 → event 매니저 → service 구현
                                                                    │
    ┌───────────────────────────────────────────────────────────────┘
    ▼
  테스트 추론으로 검증 → 반복 개선 → PR 제출
```
